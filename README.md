...
| Agents failing to create specs, claim tasks, and update artifacts once several features ran at once | The platform owns a **turn**, so it can enforce **one live turn** per artifact, **one committing write** per turn, hand each ready task to exactly one worker, **resume from a checkpoint** after a crash, tell a superseded turn to **stop before its next model call**, and **repair a malformed model output inside the turn**. Leases are sized per skill; who holds the pen is a readable row | [`design/changes-from-v6.md`](design/changes-from-v6.md) §5, [`design/platform/turn-queue.md`](design/platform/turn-queue.md) |
...
| **Turn** | One unit of worker execution: a role, a skill, a trigger, an artifact scope, a budget, a lease, a result. The platform queues turns; workers claim them. A turn makes **one** committing write, checkpoints as it goes, and stops when its heartbeat says it is superseded |
