...
| 2 | Turn queue and workers | `turns`, claim protocol, per-turn tokens, enqueue rules, one-live-turn index, single-committing-write enforcement, checkpoints and resume, heartbeat currency and cancellation, per-skill lease sizing on a reserved connection allowance, slots, stand-downs, reaper, skill and worker registries, a reference worker runtime | `platform/turn-queue.md`, `roles.md`, `governance.md` |
...
| 6 | Specify | intent capture and the `MAX_OPEN_FEATURES` intake queue, `specify` and `clarify` skills, clarification sessions, Product Owner review, bounds | `pipeline/phase-1-specify.md`, `ledger/feature-spec.md`, `ledger/clarification.md` |
...
| 14 | Human UI | inboxes, feature dashboard with derived phase and `waiting_on`, decision and answer forms, board, blockers, merge queue view, the `pen_holder` view and turn retry, intake queue, audit timeline, admin surfaces | `platform/overview.md`, `platform/projections.md` |
